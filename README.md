...
[![Docker Version](https://img.shields.io/badge/version-4.0.0-blue)](https://hub.docker.com/repository/docker/openhab/openhab/tags?name=4.0.0)
...
If you are unsure about what your needs are, you probably want to use `openhab/openhab:4.0.0`.
...
`docker run --name openhab --net=host openhab/openhab:4.0.0`
...
`docker run --detach --name openhab openhab/openhab:4.0.0`
...
`docker run -it openhab/openhab:4.0.0`

#### Debug mode

The debug mode is started with the command:

`docker run -it openhab/openhab:4.0.0 ./start_debug.sh`
...
```shell
$ docker build --build-arg JAVA_VERSION=11 --build-arg OPENHAB_VERSION=4.0.0 --tag openhab/openhab .
...
```
$ docker buildx build --build-arg JAVA_VERSION=11 --build-arg OPENHAB_VERSION=4.0.0 --platform linux/arm/v7 --tag openhab/openhab --load .
```

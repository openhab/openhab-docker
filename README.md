...
[![Docker Version](https://img.shields.io/badge/version-4.3.6-blue)](https://hub.docker.com/repository/docker/openhab/openhab/tags?name=4.3.6)
...
* **Milestone:** Intermediary releases of the next openHAB version which are released about once a month. They include recently added features and bugfixes and are a good compromise between the current stable version and the bleeding-edge and potentially unstable snapshot version.
  * `5.0.0.RC1` ([Release notes](https://github.com/openhab/openhab-distro/releases/tag/5.0.0.RC1))
  * `5.0.0.M4` ([Release notes](https://github.com/openhab/openhab-distro/releases/tag/5.0.0.M4))
...
If you are unsure about what your needs are, you probably want to use `openhab/openhab:4.3.6`.
...
`docker run --name openhab --net=host openhab/openhab:4.3.6`
...
`docker run --detach --name openhab openhab/openhab:4.3.6`
...
`docker run -it openhab/openhab:4.3.6`

#### Debug mode

The debug mode is started with the command:

`docker run -it openhab/openhab:4.3.6 ./start_debug.sh`
...
```shell
$ docker build --build-arg JAVA_VERSION=17 --build-arg OPENHAB_VERSION=4.3.6 --tag openhab/openhab .
...
```shell
$ docker buildx build --build-arg JAVA_VERSION=17 --build-arg OPENHAB_VERSION=4.3.6 --platform linux/arm64 --tag openhab/openhab --load .
```

...
* `2.5.4-snapshot` Experimental openHAB 2.5.4 SNAPSHOT version ([Dockerfile](https://github.com/openhab/openhab-docker/blob/master/2.5.4-snapshot/debian/Dockerfile-amd64))
...
If you are unsure about what your needs are, you probably want to use `openhab/openhab:2.5.3`.
...
`docker run --name openhab --net=host openhab/openhab:2.5.3`
...
`docker run --detach --name openhab openhab/openhab:2.5.3`
...
`docker run -it openhab/openhab:2.5.3`

#### Debug mode

The debug mode is started with the command:

`docker run -it openhab/openhab:2.5.3 ./start_debug.sh`
